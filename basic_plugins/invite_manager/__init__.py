...
text = f"想要邀请{NICKNAME}入群嘛？以下为目前的入群条件(虽然很长，但是不看完不接受拉群)：\n\n"\
    ...
    if Config.get_config("invite_manager", "AUTO_ADD_FRIEND", False) and bot.self_id == str(MAIN_BOT):
        await bot.send_private_msg(
            ...
            message="已自动通过你的好友申请啦，这里是奏宝一号机\n"
                    "然而咱并不开放私聊功能噢，想使用功能还请在群聊中触发呢\n"
                    "若有拉群需求，发送文本：入群条件 可获得入群要求的说明\n"
                    "如果需要咨询咱的master请通过指令沟通👉👉发送格式：滴滴滴 这里是你想说的话"
        )
    else:
        await bot.send_private_msg(
            user_id=event.user_id,
...
async def _(bot: Bot, event: FriendRequestEvent):
    ...
    # 自动添加好友，更新好友信息
    if Config.get_config("invite_manager", "AUTO_ADD_FRIEND") and bot.self_id == str(MAIN_BOT):
        await bot.set_friend_add_request(flag=event.flag, approve=True)
        await FriendUser.add_friend_info(user["user_id"], user["nickname"])
    elif Config.get_config("invite_manager", "AUTO_ADD_FRIEND") and flag:
        await bot.set_friend_add_request(flag=event.flag, approve=True)
        await FriendUser.add_friend_info(user["user_id"], user["nickname"])
    # 添加到请求管理器
    else:
