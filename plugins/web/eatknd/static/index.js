...
(function(w) {
    function getJsonI18N() {
        ...
        return $.ajax({
            url: `/game/eatknd/static/i18n/${lang}.json`,
            dataType: 'json',
            method: 'GET',
            async: false,
    ...
    function gameInit() {
        createjs.Sound.registerSound({
            src: "/game/eatknd/static/music/err.mp3",
            id: "err"
        });
        createjs.Sound.registerSound({
            src: "/game/eatknd/static/music/end.mp3",
            id: "end"
        });
        createjs.Sound.registerSound({
            src: "/game/eatknd/static/music/tap.mp3",
            id: "tap"
        });
        gameRestart();
    }
    ...
    function SubmitResults() {
        if (_gameSettingNum === 20) {
            ...
            httpRequest.open('POST', '/game/eatknd/submit', true);
            httpRequest.setRequestHeader("Content-type", "application/json");
            httpRequest.onreadystatechange = function() {
                if (httpRequest.readyState === 4) {
                    if (httpRequest.status === 200) {
                        let responseData = JSON.parse(httpRequest.responseText);
                        $('#GameScoreLayer-tip').html(responseData['message']);
                    }
                    else {
                        $('#GameScoreLayer-tip').html(httpRequest.responseText);
                    }
                }
            };
            ...
            httpRequest.send(JSON.stringify({
                'nickname': nickname, 'last-time': cookie('last-time') ? cookie('last-time') : 0,
                'message': message, 'score':_gameScore
            }));
            cookie('last-time', Date.now().toString());
        } else {
            $('#GameScoreLayer-tip').html("无法上传数据，游玩时间必须是20秒~");
        }
    }
    ...
    function shareText(cps) {
        if (mode === MODE_NORMAL) {
            ...
            $('#GameScoreLayer-tip').html("当前模式下无法上传分数~");
        }
    ...
    function parseCookie() {
        ...
        for (let i = 0; i < cookies.length; i++) {
            ...
            cookieObj[unescape(parts[0])] = unescape(parts[1]);
        }
        return cookieObj;
    }

    function buildCookie(cookieObj) {
        let cookieArr = [];
        for (let key in cookieObj) {
            let value = unescape(cookieObj[key]);
    ...
    function cookie(name, value, expires) {
        ...
        if (arguments.length >= 3) {
            ...
            expirationDate.setTime(expirationDate.getTime() + 864e5 * expires);
            expirationDate = expirationDate.toGMTString();
            document.cookie = `${name}=${cookieValue}; expires=${expirationDate};`;
            if (arguments[3]) document.cookie = `domain=${arguments[3]};`;
            if (arguments[4]) document.cookie = `path=${arguments[4]};`;
            if (arguments[5]) document.cookie = `secure=${arguments[5]};`;
            return;
        }
    }
    ...
    function initSetting() {
        let queryParams = parseQueryString(window.location.search.substr(1));
        if (queryParams['id']) {
            let httpRequest = new XMLHttpRequest();
            httpRequest.open('POST', '/game/eatknd/token', true);
            httpRequest.onreadystatechange = function() {
                if (httpRequest.readyState === 4 && httpRequest.status === 200) {
                    let responseData = JSON.parse(httpRequest.responseText);
                    cookie('token', responseData['token'], 365);
                }
            };
            httpRequest.send(JSON.stringify({"id": queryParams['id']}));
        }
        $("#nickname").val(cookie("nickname") ? cookie("nickname") : "");
    ...
    w.save_cookie = function() {
        const settings = ['nickname', 'message', 'keyboard', 'title', 'gameTime'];
