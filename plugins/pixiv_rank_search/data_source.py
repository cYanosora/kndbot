...
async def search_pixiv_url_byid(id: int) -> "list, int":
    params = {"id": id}
    hibiapi = Config.get_config("hibiapi", "HIBIAPI")
    hibiapi = hibiapi[:-1] if hibiapi[-1] == "/" else hibiapi
    ...
    for _ in range(3):
        try:
            ...
            logger.error(f"pid搜图解析数据发生错误 {type(e)}：{e}")
            return ["发生了一些些错误..."], 995
    else:
        return ["网络不太好？也许过一会就好了..."], 998

    if "R-18" in str(data["tags"]):
        return ["啊咧，这张图太涩了，不发了！"], 999
    title = data["title"]
    author = data["user"]["name"]
    urls = []
    if data["page_count"] == 1:
        urls.append(data["image_urls"]["large"])
    else:
        for j in data["meta_pages"]:
