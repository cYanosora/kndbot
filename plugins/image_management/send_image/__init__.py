...
usage：
    ...
    注意：
        若发现图库中的图有分类错误的现象，方便的话，请使用你一点点时间通过 滴滴滴 功能来告知master
        烧烤角色的昵称管理详见 '角色昵称/别名' 功能(便于使用角色称呼发图)
        此功能主要用于指路，为提高发图速度，部分图已经受压制处理，有存图需求还请从图源处获取(重要)
        质量参差不齐、画风多变请见谅，数量相差过大与个人偏好以及同人圈热度有关
        男女混团含bg图容易开幕雷击(过激厨、不拆不逆挑食人请另寻妈咪)，另外cp图库中实际含不少cb图
        有收藏不当的图可以联系master删除，可以使用 '滴滴滴' 功能联系到master
        限制群内1分钟内最多发送4张，单人每日最多看25张，防止刷屏(´∀｀；)(有用吗喂)
""".strip()
__plugin_settings__ = {
    "cmd": ["看图", "本地图库"]
}
