...
__plugin_name__ = "pix检查更新 [Superuser]"
__plugin_type__ = "PIX"
...
start_update = on_command(
    "更新pix关键词", aliases={"更新pix关键字"}, permission=SUPERUSER, priority=4, block=True
)

check_not_update_uid_pid = on_command("pix检测更新", aliases={"pix检查更新"},
                                      permission=SUPERUSER, priority=4, block=True,)

check_omega = on_command("检测omega图库", permission=SUPERUSER, priority=4, block=True)
...
async def _(arg: Message = CommandArg()):
    ...
    if num < 10000:
        ...
        if msg.lower() == "pid":
            ...
            info = f"开始更新Pixiv搜图关键词：\n{keyword_str}"
            info += f"\n更新UID：{uid_str}" if uid_str else ""
            info += f"\n更新PID：{pid_str}" if pid_str else ""
