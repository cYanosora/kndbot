...
from ._data_source import getchart
try:
    import ujson as json
except:
    import json

__plugin_name__ = "谱面预览"
...
# preview
preview = on_keyword({'谱面预览'}, permission=GROUP, priority=5, block=True)
...
async def _(event: GroupMessageEvent):
    ...
    if not arg:
        await preview.finish("使用方法：谱面预览 [难度] [曲名]")
    diff_dict = {
    ...
    if data['match'] < 0.6 or data['musicId'] == 0:
        ...
        try:
            ...
            await preview.finish("获取失败！可能是バグ！")
            return
        else:
            if dir:
                ...
                if 'SekaiViewer' in str(dir):
                    ...
                    text += '\nBPM: ' + bpmtext[3:] + '\n谱面图片来自ぷろせかもえ！'
                await preview.finish(text + image(dir))
            else:
