...
from configs.path_config import TEMP_PATH, FONT_PATH
from services import logger
from utils.http_utils import AsyncHttpx
from utils.imageutils import pic2b64
from .._autoask import pjsk_update_manager
from .._config import data_path
from .._models import PjskSongsAlias, MusicInfo
import datetime
import os
import time
import pytz
...
async def info(musicid) -> Tuple[bool, str]:
    path = TEMP_PATH / f'pjskinfo_{musicid}.png'
    if path.exists():
        ...
        if pjskinfotime > playdatatime:  # 缓存后数据未变化
            if time.time() < publishedAt:  # 捷豹
                ...
                if pjskinfotime.timestamp() < publishedAt:  # 缓存是上线前的
                    return await drawpjskinfo(musicid, False)
                return False, ""
        else:
            return await drawpjskinfo(musicid, False)
    else:
...
async def musiclength(musicid, fillerSec=0):
    try:
...
async def drawpjskinfo(musicid: int, olddir=True) -> Tuple[bool, str]:
    info = MusicInfo()
    ...
    # 长度(需要mutagen.mp3)
    info.length = await musiclength(musicid, info.fillerSec)
    if info.length:
    ...
    if olddir:
        img.save(TEMP_PATH / fr'pjskinfo_{musicid}.png')
    else:
        img.save(TEMP_PATH / fr'pjskinfo_{musicid}.png')
