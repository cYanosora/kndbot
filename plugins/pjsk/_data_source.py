...
from plugins.pjsk._models import PjskSongsAlias, MusicInfo
import datetime
import os
import time
import pytz
        ...
        if pjskinfotime > playdatatime:  # 缓存后数据未变化
            if time.time() < publishedAt:  # 偷跑
                ...
                if pjskinfotime.timestamp() < publishedAt:  # 缓存是上线前的
                    return await drawpjskinfo(musicid)
                return False, ""
        else:
            return await drawpjskinfo(musicid)
    else:
...
def matchname(alias):
    ...
    for musics in data:
        ...
        if similar > match['match']:
            # print(f'找寻到新曲相似度更高: {musics["titles"]}>{match["title"]}')
            match['match'] = similar
        ...
        try:
            ...
            if '/' in translate:
                ...
                for i in alltrans:
                    ...
                    if similar > match['match']:
                        # print(f'从翻译中找寻到新曲相似度更高: {musics["titles"]}>{match["title"]}')
                        match['match'] = similar
                        match['musicId'] = musics['id']
                        match['title'] = musics['title']
            else:
...
# 生成pjskinfo图片的具体函数
async def drawpjskinfo(musicid: int) -> Tuple[bool, str]:
    info = MusicInfo()
